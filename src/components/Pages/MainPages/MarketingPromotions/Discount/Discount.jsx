...
const Discount = () => {
  ...
  const allDiscounts = useSelector((state) => state.cart.allDiscounts);
  const getAllDiscountsHere = async () => {
    try {
      const res = await axios.get(`${APIBASE}admin/discounts?page=${page}`);
  ...
  const handleExportExcel = () => {
    const exportIt = listItems?.map((elem, index) => ({
      "Sr. No.": index + 1,
      email: elem.email,
    }));
    ...
    if (event) {
      try {
        const response = await axios.put(
          `${APIBASE}admin/update-discount-status/${id}`,
          { status: 1 }
        );
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <div style={{ display: "flex" }}>
          ...
          <h6 style={{ margin: "5px" }}>
            Promotions - Discounts - Periodic Discounts
          </h6>
        </div>
                      ...
                      <TableRow>
                        ...
                        <TableCell>
                          ...
                          >
                            <MenuItem>
                              <Link
                                to={`/admin/marketing-promotions/edit-periodic-discount/${row.id}`}
                            ...
                            <MenuItem onClick={() => deleteDiscount(row.id)}>
                              <small>
