...
const BackOrder = () => {
  ...
  const getFilteredOrders = async () => {
    ...
    try {
      const res = await axios.get(
        `${APIBASE}admin/get-back-order?page=${page}${params.search.replace(
          /\?/g,
          "&"
        )}`
      );
  ...
  const getSingleOrder = async () => {
    try {
      const res = await axios.get(
        `${APIBASE}admin/get-back-order?search=${searchOrder}`
      );
      setListItems(res.data.data.data);
  ...
  const moveToOrder = async (id) => {
    handleMenuClose(true);
    try {
      await axios.put(`${APIBASE}admin/change-order-status/${id}`, {
        back_order: 2,
      });
      toast.success("Moved to Order successfully.");
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <div style={{ display: "flex" }}>
          ...
          <h6 style={{ margin: "5px" }}>Sales - Back Orders</h6>
        </div>
          ...
          <div className="orders-section">
            ...
            <div className="order-head">
              <Typography variant="h1">
                All Back Orders (Total:-{totalItems})
              </Typography>
                          ...
                          <React.Fragment key={index}>
                            <TableRow>
                              ...
                              Edit
                                  ...
                                  <MenuItem onClick={() => moveToOrder(row.id)}>
                                    <small>
                                      <ShoppingCartIcon
                                        sx={{ marginRight: 1 }}
                                      />
