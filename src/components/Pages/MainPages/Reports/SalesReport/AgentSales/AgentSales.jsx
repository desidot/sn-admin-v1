...
import { getAllEmployees } from "../../../../../../redux/cartSlice";
...
const AgentSalesReport = () => {
  ...
  const [agentId, setEmployee] = useState("");

  const getAllSalesReport = async () => {
    if (from_date && to_date && agentId) {
      try {
        const res = await axios.get(
          `${APIBASE}admin/reports/get-agent-sales-report/${agentId}/${from_date}/${to_date}`
        );
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <div style={{ display: "flex" }}>
          ...
          <h6 style={{ margin: "5px" }}>
            Report - Sales Report - POS - Agent Sales Report
          </h6>
        </div>
        ...
        <div className="filter-container">
          <Grid container spacing={2}>
            <Grid item xs={12} md={3}>
              <InputLabel htmlFor=""> All Agents :</InputLabel>
              <FormControl fullWidth>
                ...
                {/* <Autocomplete
                  ...
                  id="free-solo-2-demo"
                  disableClearable
                  options={allAgents?.map(
                    (option) => `${option?.id}` + ". " + option?.name
                  )}
                  onChange={(e, newValue) => handleAddCustomer(e, newValue)}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      InputProps={{
      ...
      <div className="card">
        ...
        <h3 className="card-title">All Emplyee Report</h3>
          {/* Buttons */}
