    ...
    if (from_date && to_date) {
      try {
        const res = await axios.get(
          `${APIBASE}admin/reports/get-subscription-report/${from_date}/${to_date}`
        );
        setData(res.data);
      } catch (error) {
        //console.log(error);
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <div style={{ display: "flex" }}>
          ...
          <h6 style={{ margin: "5px" }}>
            Report - Sales Report - POS - Subscription Report
          </h6>
        </div>
                ...
                {listItems?.map((row, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell align="left">
                      ...
                      {JSON.parse(row?.billing_address).state}{" "}
                      {JSON.parse(row?.billing_address).country}
                    </TableCell>
                    <TableCell align="left">
                      <span className="test-orange">
                        {" "}
                        {JSON.parse(row?.items)[0]?.slug}
                      </span>
                    </TableCell>
                    <TableCell align="left">${row?.sub_total}</TableCell>
                    <TableCell align="left">${row?.subscription_discount}</TableCell>
